...
//	public delegate void WaitForMethod(params object[] list);
    ...
    public class Simulation
    {
        ...
        public static void Tell(TellMethod tm, double start, double priority, object[] list)
        {
            ...
            if (Scheduler.HasStoped)
                Scheduler.ResetSimTime();
...
//			Console.WriteLine("		===Simulation.Wait()========================>, Wait Time: "+t);
        ...
        public static void WaitFor(GiveMethod gm, object obj, int numberDesired, ref bool interrupt, double priority = 0.0)
        {
            bool rc = gm(obj,numberDesired, priority);
            if(!rc)
            {
                //Aktuelles Objekt aus der Liste holen, ist Objekt das Arbeit suspendieren muss
                SimObj currObj = Scheduler.GetCurrentSchedObj();
        ...
        /// <param name="interrupt">Interrupt</param>
        public static void WaitFor(TriggerObj trigger, object obj, ref bool interrupt)
        {
            SimObj currObj = Scheduler.GetCurrentSchedObj();
            currObj.Waitfor = trigger;
            
            trigger.WaitingObjects.Add(currObj);
...
//			Console.WriteLine("In Simulation.InterruptAll()"); 
        ...
        public static void StopSimulation()
        {
            ...
            Scheduler.RemoveAllSchedObj();
            Scheduler.RemoveAllWaitForObj();
        }
    }
}
